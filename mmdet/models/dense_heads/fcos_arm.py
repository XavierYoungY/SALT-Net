...
class NFHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.scales_init = nn.ModuleList([Scale(1.0) for _ in self.strides])
    ...
    def forward(self, feats):
        ...
        return multi_apply(self.forward_single, feats, self.scales,
                           self.strides, self.scales_init)

    def forward_single(self, x, scale, stride, scale_init):
        ...
        if self.training:
            return cls_score, bbox_pred, centerness, bbox_distance
        else:
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'bbox_distances')
    )
    def loss(self,
             cls_scores,
             bbox_preds,
        ...
        flatten_bbox_distances = [
            bbox_distance.permute(0, 2, 3, 1).reshape(-1, 4)
            for bbox_distance in bbox_distances
        ]
        flatten_centerness = [
        ...
        flatten_bbox_distances = torch.cat(flatten_bbox_distances)
        ...
        pos_bbox_distances = flatten_bbox_distances[pos_inds]
        ...
        if num_pos > 0:
            ...
            #YY
            pos_decoded_bbox_distances = distance2bbox(pos_points,
                                                       pos_bbox_distances)
            ...
            loss_bbox_init = pos_bbox_distances.sum()
